...
3. Azure SQL データベース エンドポイントの仮想ネットワークの IP 範囲を構成する
    ...
    - **default**のサブネットをクリックします。 (表示されるサブネット アドレス範囲は異なる場合があります。)  
...
4. [SQL Database の作成] 画面の基本タブに次の情報を入力し、**[次へ: ネットワーク]** をクリックします
    ...
    - サーバー: **[新規作成]** をクリックします。 New Server サイドバーで、フォームに次のように入力して**[OK]**を押します。
        ...
        **注:このサーバー名とログイン情報を書き留めます。これは後のラボで使用します。**
        ...
        SQL Database サーバーの作成のサイドバーは次のようになります。
...
5. **[バックアップ ストレージの冗長性]** オプションが表示された場合は、既定値:**geo 冗長バックアップ ストレージ** のままにします。 
...
9. 次のように、[プライベート エンドポイントの作成] 画面で以下の情報を入力して **OK** をクリックします。
...
10. プライベート エンドポイントが [ネットワーク] ページに表示されていることを確認します。 
...
3. **[パブリック ネットワーク アクセス]** を **[選択したネットワーク]** に変更して **[Azure サービスとリソースがこのサーバーにアクセスすることを許可する]** を **[はい]** に設定します  
...
4. **[リソースの種類]** を **[単一サーバー]** に変更して **作成** をクリックします  
...
6. Azure Data Studio の PostgreSQL Database への 接続を確認してください。  
