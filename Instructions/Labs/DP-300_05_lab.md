...
5. AdventureWorks データベースに対してこのクエリを実行します。 
...
# <a name="exercise-2-isolate-problem-areas-in-poorly-performing-queries-in-adventureworks2017"></a>演習 2:AdventureWorks でパフォーマンスの低いクエリの問題領域を特定します
...
FROM AdventureWorks.Sales.SalesOrderDetail WHERE ModifiedDate > '2012/05/01' AND ProductID = 772;
```
...
INCLUDE ([CarrierTrackingNumber],[OrderQty],[SpecialOfferID],[UnitPrice],[UnitPriceDiscount],[LineTotal],[rowguid])
...
# <a name="exercise-3-use-query-store-to-detect-and-handle-regression-in-adventureworks2017"></a>演習 3:クエリ ストアを使用して、AdventureWorks の機能低下を検出して処理します。
...
1. 以下のコードをコピーして新しいクエリ ウィンドウに貼り付け、**master** データベースに対して実行します。 このスクリプトは、AdventureWorks2017 のクエリ ストアを有効にし、データベースの互換性レベルを 100 に設定します
...
2. SQL Server Management Studio のメニューで、[ファイル] > [開く] > [ファイル] の順に選択します。 D:\Allfiles\Labs\Query Performance\CreateRandomWorkloadGenerator.sql ファイルに移動します。 ファイルをクリックして Management Studio にロードし、**AdventureWorks** データベースに対してクエリを実行します。
...
4. 以下のコードをコピーして新しいクエリ ウィンドウに貼り付け、**master** データベースに対して実行します。 このスクリプトを使用すると、データベース互換モードが SQL Server 2019 (150) に変更されます。
...
1. Management Studio で AdventureWorks データベースの下に [クエリ ストア] オプションが表示されます。  
...
3. **[プランの強制]** をクリックします。 確認ウィンドウが表示されたら、[はい] を選択してプランを強制的に適用します。
...
‎
2. 次のクエリを **AdventureWorks** データベースに対して実行します。 実行プランでインデックス シーク オペレーターが示されていることに注意してください。
...
2. WHERE 句でハードコードされた値の代わりに、検索する値をパラメーターとして渡すことができるように、パラメーター化されたストアド プロシージャを作成します。 パラメーターのデータ型がターゲット テーブルの列のデータ型と一致することを確認する必要があります。 次のコードをコピーして **AdventureWokrs** データベースに対して実行します。 
...
5. 次のコマンドを実行して、AdventureWorks データベースのプラン キャッシュをクリアします。
